...
def _server_payload(config_dir: str | None = None) -> dict:
    env = {}
    if config_dir:
...
class MCPRemoteAuthTests(unittest.TestCase):
    ...
    def test_poll_contract(self):
        ...
        with patch("sandbox_compute_server.mcp_remote_auth._discover_tools", new=_wait_for_code):
            ...
            self.assertIn(submitted["status"], {"code_submitted", "authorized"})
    ...
    def test_start_authorize_completion_lifecycle(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            async def _wait_for_code(runtime):
                for _ in range(500):
                    session = mcp_remote_auth._MANAGER.get(session_id)
                    if session and session.auth_code:
                        config_dir = Path(runtime["env"]["MCP_REMOTE_CONFIG_DIR"]) / "mcp-remote-0.1.40"
                        config_dir.mkdir(parents=True, exist_ok=True)
                        (config_dir / "abc_tokens.json").write_text(
            ...
            with patch("sandbox_compute_server.mcp_remote_auth._discover_tools", new=_wait_for_code):
                started = mcp_remote_auth.handle_remote_auth_start(
                    {
                        "session_id": session_id,
                        "server": _server_payload(config_dir=temp_dir),
                ...
                self.assertIn("remote_auth_state", status_payload)
                files = status_payload["remote_auth_state"].get("files") or []
                self.assertTrue(any(item.get("path", "").endswith("abc_tokens.json") for item in files))
    ...
    def test_hydrate_and_snapshot_runtime_remote_auth_state(self):
        with TemporaryDirectory() as temp_dir:
            runtime = {
                "env": {"MCP_REMOTE_CONFIG_DIR": temp_dir},
                "remote_auth_state": {
                    "version": 1,
                    "files": [
                        {
                            "path": "mcp-remote-0.1.40/server_tokens.json",
                            "content_b64": "eyJhY2Nlc3NfdG9rZW4iOiJ0b2tlbiJ9",
                        }
                    ],
                },
            }
            mcp_remote_auth.hydrate_runtime_remote_auth_state(runtime)

            expected = Path(temp_dir) / "mcp-remote-0.1.40" / "server_tokens.json"
            self.assertTrue(expected.exists())

            snapshot = mcp_remote_auth.snapshot_runtime_remote_auth_state(runtime)
            files = snapshot.get("files") or []
            self.assertTrue(any(item.get("path") == "mcp-remote-0.1.40/server_tokens.json" for item in files))
